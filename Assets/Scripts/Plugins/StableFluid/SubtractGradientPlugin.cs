...
{
    public class SubtractGradientPlugin : MonoBehaviour, IPlugin
    {
        public string Identifier => this.ToString();
        public bool Enabled => this.isActiveAndEnabled;
        public IEnumerable<int> Steps => new List<int>() { (int)SimulationStep.OnSimulation + (int)Step.SubtractGradient };
        ...
        public void OnSimulationStep(int stepIndex, ISimulation sim, ISimulationData data)
        {
            ...
            this.SubtractGradient(mat, velocity.Read.Data, pressure.Read.Data, velocity.Write.Data, new float3(1) / velocity.Read.Size);
            velocity.SwipeBuffer();
        }

        protected void SubtractGradient(Material mat, RenderTexture velocity, RenderTexture pressure, RenderTexture dest, float3 invSize)
        {
            mat.SetVector("_InverseSize", new Vector4(invSize.x, invSize.y, invSize.z));
            mat.SetTexture("_Velocity", velocity);
            mat.SetTexture("_Pressure", pressure);
